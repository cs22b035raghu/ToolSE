...
def generate_openrouter_suggestions1(summary_text):
    prompt = f"""answer the question or respond if he greets :\n\n{summary_text}"""
...
def adjusted_similarity_score(module_name, pdf_text, title_text, report_summary):
    # Normalize whitespace: strip, remove extra spaces/newlines
    def clean_text(text):
    ...
    print("\n=== Cleaned PDF Text ===\n", pdf_text[:1000], "...\n")  # printing only the first 1000 characters
    print("=== Cleaned Title Text ===\n", title_text, "\n")
    print("=== Cleaned Report Summary ===\n", report_summary, "\n")

    # If module name not found in either title or pdf, return low similarity
...
def chat():
    ...
    # Here, you can replace this part with your logic to generate AI response
    # For now, let's just simulate an AI response.
    
    # You could use the model or an API for AI response generation.
    #ai_response = f"Echo: {user_message}"  # Simple echo for demonstration
...
{all_code[:12000]}  # Truncate to keep prompt size reasonable
"""
